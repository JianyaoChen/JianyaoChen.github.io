---
permalink: /
title: "Jianyao Chen"
author_profile: true
redirect_from:
  - /about/
  - /about.html
---

## About Me

I am a **Ph.D. Candidate in Information Science** at the School of Information Studies, University of Wisconsin–Milwaukee. 

My doctoral research examines **PTSD-related discussions in online health communities**. Using a mixed-methods approach, I investigate how individuals express emotions, seek and provide social support, manage privacy-related self-disclosure, and encounter misinformation in digital mental health contexts. Methodologically, my work integrates computational text analysis and quantitative research methods, including large-scale social media data collection, natural language processing, deep learning, large language models, and statistical modeling. 


## Research Interests

- Digital Health
- Health Informatics
- Online Information Behavior
- Online Social and Emotional Support
- Digital Well-Being
- Privacy-Related Concerns on Digital Platforms,
- Misinformation
- Health Informatics
- Deep Learning
- Large Language Models


## Education

**Ph.D. in Information Science**  
University of Wisconsin–Milwaukee (UWM), 2022.09-2026.06 (expected)

**M.S. in Information Science**  
Central China Normal University (CCNU), 2019.09–2022.06  

**B.S. in Information Management and Information Systems**  
Central China Normal University (CCNU), 2015.09–2019.06  
 

## Technical & Research Skills

**Programming & Data Processing**  
Python (Pandas, NumPy, Scikit-learn, Selenium); API-based data collection; large-scale text data cleaning

**Natural Language Processing**  
Text embedding; topic modeling (BERTopic, LDA); sentiment and emotion analysis; named entity extraction

**Deep Learning & AI**  
TensorFlow; large language model integration using LangChain; prompt-based system design

**Research Methods**  
Content analysis; statistical analysis; mixed methods


